...
const TrackingPage = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      // Query the public view to avoid exposing email
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          ...
          >
            ...
            <h1 className="font-display font-extrabold text-4xl md:text-5xl lg:text-6xl text-surface-foreground mb-4">
              Suivez votre <span className="text-gradient-primary">colis</span>
            </h1>
            <p className="text-surface-foreground/60 text-lg max-w-xl mx-auto">
          ...
          >
            <div className="flex gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  placeholder="Ex: IE-2025-00142"
                  className="w-full bg-surface-foreground/5 border border-surface-foreground/10 rounded-xl pl-12 pr-4 py-4 text-lg font-mono text-surface-foreground placeholder:text-surface-foreground/25 focus:outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all"
                />
              </div>
              <Button
                type="submit"
            ...
            <p className="text-surface-foreground/30 text-xs mt-3 text-center">
              Essayez: IE-2025-00142 ou IE-2025-00198
            </p>
          </motion.form>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {error && !loading && (
            ...
            >
              ...
              <h3 className="font-display font-bold text-xl text-foreground mb-2">Colis introuvable</h3>
          ...
          {shipment && !loading && cfg && (
            ...
            >
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-sm font-semibold ${cfg.bg} ${cfg.color}`}>
                        <cfg.icon size={14} />
                        {cfg.label}
                      </span>
                    </div>
                  ...
                  {shipment.estimated_delivery && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted rounded-lg px-4 py-2">
                      <Clock size={16} />
                      <span>Livraison estimée: <strong className="text-foreground">{new Date(shipment.estimated_delivery).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</strong></span>
                    </div>
                  )}
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <InfoCard icon={MapPin} label="Origine" value={shipment.origin} />
                  <InfoCard icon={MapPin} label="Destination" value={shipment.destination} />
                  <InfoCard icon={Package} label="Type" value={shipment.cargo_type || "—"} />
                  <InfoCard icon={Weight} label="Poids" value={shipment.weight_kg ? `${shipment.weight_kg} kg` : "—"} />
                </div>
              </div>
              ...
              {events.length > 0 && (
                <div>
                  <h3 className="font-display font-bold text-xl text-foreground mb-8">Historique de l'expédition</h3>
